...
function sliderCtaHover() {
	...
	gsap.set(image, { rotationY: -55, opacity: 0 });
	...
	ctas.forEach((cta) => {
		...
		cta.addEventListener("mouseenter", () => {
			...
			// image in
			gsap.to(image, { opacity: 1, rotationY: 0, transformOrigin: "50% 50% -890", duration: 0.55, ease: "myEaseSmooth", delay: 0.1 });
		});
		...
		cta.addEventListener("mouseleave", () => {
			...
			// image out
			gsap.to(image, { opacity: 0, rotationY: 55, transformOrigin: "50% 50% -890", duration: 0.55, ease: "myEaseSmooth", delay: 0 });

			// titles in
			gsap.to([titles, arrows, description], { opacity: 1, duration: 0.55, ease: "myEaseSmooth", delay: 0.3 });

			// reset image position
			gsap.set(image, { rotationY: -55, opacity: 0, duration: 0, delay: 0.57 });
		});
	});
}
